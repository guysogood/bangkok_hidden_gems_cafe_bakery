...
Task graph:
    ...
    bronze_to_silver >> validate_silver >> silver_to_gold >> validate_gold >> load_to_bigquery
...
Validation tasks act as quality gates: if either validator exits non-zero the DAG
task is marked failed and downstream tasks are blocked automatically.
"""
...
with DAG(
    ...
    validate_silver = BashOperator(
        ...
        bash_command=(
            f"{PYTHON_BIN} {PROJECT_ROOT}/src/validate/validate_silver.py "
            f"--path {SILVER_PATH}"
        ),
    )
    ...
    validate_gold = BashOperator(
        ...
        bash_command=(
            f"{PYTHON_BIN} {PROJECT_ROOT}/src/validate/validate_gold.py "
            f"--path {GOLD_PATH}"
        ),
    )
